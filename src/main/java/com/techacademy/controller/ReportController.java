...
public class ReportController {
    ...
    /** 日報一覧画面を表示（ログインユーザ本人分） */
    @GetMapping("/")
    public String getMyList(@AuthenticationPrincipal UserDetail user,Model model) {
        // ログインユーザ本人分の結果のみをModelに登録
        model.addAttribute("myreportlist", service.getMyReportList(user));
    ...
    public String getReport(@PathVariable("id") Integer id,@AuthenticationPrincipal UserDetail user,Model model) {
    ...
    public String updateReport(@PathVariable("id") Integer id, @AuthenticationPrincipal UserDetail user, Model model) {
        model.addAttribute("report", service.getReport(id));
        // 同一ユーザかどうかをチェック
        if(service.checkEqualID(id,user)==true) {
            //trueなら更新画面に遷移
